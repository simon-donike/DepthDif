...
class StructureFunctionPriorLoss(nn.Module):
    ...
    def _sample_pair_stats(
        ...
        squared = increments.square()
        num_bins = int(self.distance_bins.numel()) - 1
        if self.per_depth:
            flat_idx = c_idx * num_bins + bin_idx
            sums = torch.zeros(channels * num_bins, device=device, dtype=x0_pred.dtype)
        ...
        sums = torch.zeros(num_bins, device=device, dtype=x0_pred.dtype)
        counts = torch.zeros_like(sums)
