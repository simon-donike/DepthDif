# Example:
# /work/envs/depth/bin/python -m depth_recon.inference.api infer-week --year 2015 --iso-week 25 --rectangle -20 30 10 50 --device cuda --config-repo simon-donike/DepthDif
...
def resolve_hf_land_mask(
    *,
    ...
    try:
        ...
        packaged_candidates = (
            package_root / str(land_mask_path),
            package_root
            / "data"
            / "dataset_creation"
            / "data_download_raw"
            / "get_world"
            / Path(land_mask_path).name,
        )
        for packaged_land_mask in packaged_candidates:
