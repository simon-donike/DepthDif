...
def _batch_with_sampler(
    batch: dict[str, Any], sampler: torch.nn.Module, *, progress: bool
) -> dict[str, Any]:
    """Attach one sampler to a shallow batch copy."""
    out = dict(batch)
    out["sampler"] = sampler
    out["verbose"] = bool(progress)
    return out
...
def _prediction_for_sampler(
    ...
    *,
    ...
    setattr(sampler, "progress_desc", label)
    torch.manual_seed(int(seed))
    if torch.cuda.is_available():
        torch.cuda.manual_seed_all(int(seed))
    with torch.no_grad():
        return model.predict_step(
            _batch_with_sampler(batch, sampler, progress=progress), batch_idx=0
        )
...
def main() -> None:
    ...
    pred_ddim_by_steps: dict[int, dict[str, Any]] = {}
    sampler_progress = tqdm(
        sampler_jobs,
        desc="Running samplers",
        unit="sampler",
        disable=not progress_enabled,
    )
    for label, steps, sampler in sampler_progress:
        sampler_progress.set_postfix_str(label)
