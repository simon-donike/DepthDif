...
# Reprocessed global L4 sea-level product requested by user (product ID):
# SEALEVEL_GLO_PHY_L4_MY_008_047
# `copernicusmarine get -i` expects a dataset ID.
if [[ -n "${SEALEVEL_DATASET_ID:-}" ]]; then
  # When user provides an explicit dataset ID, use only that value.
  SEALEVEL_DATASET_CANDIDATES=("${SEALEVEL_DATASET_ID}")
else
...
mkdir -p "${OUTPUT_DIR}"
CSV_LOG_PATH="${CSV_LOG_PATH:-${OUTPUT_DIR}/sealevel_daily_download_log.csv}"
...
echo "Using sea-level dataset candidates: ${SEALEVEL_DATASET_CANDIDATES[*]}"
...
while [[ "${current_date}" < "${END_DATE}" || "${current_date}" == "${END_DATE}" ]]; do
  ...
  sealevel_filter="*/${year}/${month}/*${day_tag}*.nc"

  dry_output=""
  dry_error=""
  used_dataset_id=""
  for candidate_id in "${SEALEVEL_DATASET_CANDIDATES[@]}"; do
