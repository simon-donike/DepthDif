...
For multi-depth outputs, the implementation first computes the per-channel
standard deviation and then collapses output/depth channels to a single
\(B \times 1 \times H \times W\) uncertainty raster for display and downstream
analysis. Joint temperature/salinity runs keep field-specific uncertainty maps
before producing their normalized display rasters.
