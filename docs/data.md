...
- Argo and GLORYS are always exported as the full GLORYS-aligned depth stack, which is currently 50 bands with the standard GLORYS depth axis; missing Argo pixels are written as `NaN`
- OSTIA and Argo are stored on disk as float32 GeoTIFFs; GLORYS is stored on disk as packed `int16` using Celsius `x100` (`0.01°C` precision) with the minimum `int16` value (`-32768`) as nodata
...
- `data/aggregate_depth_prod_valid_pixels_geojson.py` aggregates the median `argo_valid_spatial_observation_count` from the manifest by spatial patch, writes one patch polygon per aggregate to GeoJSON, and saves a world-outline heatmap PNG in the same plotting style as the Argo observation stats utility
- `data/export_ostia_argo_tiffs.py` runs the same export in parallel through a `DataLoader`, shuffles export order by default in contiguous blocks (`--shuffle`, optional `--shuffle-seed`, `--shuffle-block-size`, default `100`) so partial output spans the timeseries better without fully randomizing file access, writes TIFF triplets in worker processes, and writes the manifest periodically from the main process (`--flush-every`, default `100`) plus once at the end; manifest flushes use atomic file replacement so concurrent readers never observe a partially written CSV
- `OstiaArgoTiffDataset` (`data/dataset_ostia_argo_disk.py`) reads that manifest CSV back from disk, drops rows with a non-empty `export_skipped_reason` because those samples never produced TIFF triplets, verifies GeoTIFF alignment across the exported rasters, decodes packed GLORYS back to float32 Celsius before tensor construction, and returns `eo`, Argo `x`, GLORYS `y`, `valid_mask`, `valid_mask_1d`, a single-band GLORYS surface-support `land_mask`, `date`, plus optional `coords` and `info`
