...
```
Adjust `inference.grid.patch_stride` or `inference.grid.min_ocean_fraction` in the inference super-config for coverage changes; CLI flags such as `--patch-stride` and `--min-ocean-fraction` still override the config for one-off runs. Per-tile multi-generation is intentionally disabled.
