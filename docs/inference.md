...
## Workflow 1d: Export an Absolute-Error Dashboard
The standalone `/analysis/` dashboard data is generated by the Cesium packaging step. During packaging, the exporter reads the run summary, loads each prediction-vs-GLORYS absolute-error GeoTIFF, filters with the run land mask when available, aggregates finite ocean pixels by approximate basin and fixed 5-degree lat-lon cells by default, prepends an `All Depths` view with count-weighted average metrics across exported depths, and writes a compact JSON payload beside `globe-config.json`. It also writes a portable local HTML copy, but the primary hosted dashboard page lives at `docs/analysis/index.html` and loads the hosted `globe-config.json`, then fetches every packaged modality analysis dataset listed there.

Generated dashboard outputs in the hosted globe directory:
- `error-analysis.json`: global, basin, and grid-cell absolute-error metrics by depth, plus a first `All Depths` aggregate level
- `error-analysis.html`: portable local copy of the dashboard; the hosted standalone page is `docs/analysis/index.html`

The basin grouping is a land-filtered deterministic diagnostic bucket with a dominant basin label attached to every grid cell; it is still not a formal oceanographic boundary product. Use `--no-error-analysis` only when you explicitly want to skip this packaging output.
...
- generates `error-analysis.html` and `error-analysis.json` in the globe directory and records their URLs by default
...
- `error-analysis.html` and `error-analysis.json`: hosted error dashboard files generated from the run's absolute-error rasters
