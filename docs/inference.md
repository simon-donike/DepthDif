...
The standalone `/analysis/` dashboard data is generated by the Cesium packaging step. During packaging, the exporter reads the run summary, loads each prediction-vs-GLORYS absolute-error GeoTIFF, aggregates finite ocean pixels by approximate basin and fixed lat-lon cells, and writes a compact JSON payload beside `globe-config.json`. It also writes a portable local HTML copy, but the primary hosted dashboard page lives at `docs/analysis/index.html` and loads the hosted `globe-config.json`, then fetches every packaged modality analysis dataset listed there.

Generated dashboard outputs in the hosted globe directory:
- `error-analysis.json`: global, basin, and grid-cell absolute-error metrics by depth
- `error-analysis.html`: portable local copy of the dashboard; the hosted standalone page is `docs/analysis/index.html`

The basin grouping is an approximate deterministic diagnostic bucket, not a formal oceanographic boundary product. Use `--no-error-analysis` only when you explicitly want to skip this packaging output.
